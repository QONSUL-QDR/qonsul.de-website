...
const compile=async(source,target)=>{const input=await readFile(source,'utf8');const output=transformSync(input,{loader:'ts',format:'esm',target:'es2020'}).code.replace('"./analysis"','"./analysis.mjs"').replace('"./qonsul-logo-pdf"','"./qonsul-logo-pdf.mjs"');await writeFile(target,output);};

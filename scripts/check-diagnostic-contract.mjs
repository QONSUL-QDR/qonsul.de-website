...
const correction=await correctDiagnosticConsultationEmail({source_event_id:id,correction_token:correctionToken,new_correction_token:correctionToken,email:'neu@example.invalid'},{baseUrl:'https://cockpit.example',secret,fetchImpl:async(url,init)=>{
  ...
  assert.equal(init?.redirect,'manual'); correctionSeen=true;
  return Response.json({status:'accepted',correction_token:correctionToken},{status:202});
}});
...
assert.match(diagnosticUi,/aria-label="QONSUL-Hypothesen"/,'the QONSUL hypothesis section has an accessible public label');
assert.match(diagnosticUi,/<strong>QONSUL-Hypothesen<\/strong>/,'QONSUL hypotheses are visibly named');
assert.match(diagnosticUi,/QONSUL-Hypothese · \{suggestion\.category\}/,'each suggestion visibly uses the QONSUL hypothesis label');
assert.match(diagnosticUi,/QONSUL-Hypothesen sind Analysevorschläge und keine bestätigten Ursachen\./,'the QONSUL hypothesis disclaimer is visible');
assert.match(diagnosticUi,/Eigene Beobachtungen und QONSUL-Hypothesen bleiben getrennt\./,'the separation of own observations and QONSUL hypotheses is explicit');
for(const obsoletePublicTerm of ['KI-Hypothese','KI-Hypothesen','KI-Analyse','KI-/System-Hypothesen','ChatGPT','Claude','OpenAI'])assert.doesNotMatch(diagnosticUi,new RegExp(obsoletePublicTerm.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')),'the public Quality Diagnostic UI contains no obsolete technical AI terminology');
...
assert.match(correctionRoute,/new_correction_token: body\.nextToken/,'a successful correction rotates the single-use token');
const correctionPage=await readFile(new URL('../app/anfrage-verwalten/[token]/page.tsx',import.meta.url),'utf8');
assert.match(correctionPage,/Inhalte Ihrer Anfrage werden nicht angezeigt\./,'the correction page never displays request details');

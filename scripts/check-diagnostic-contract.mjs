...
assert.match(diagnosticUi,/\{completedAIRounds === 0 && <button[^>]+onClick=\{\(\) => addAIHypotheses\(\)\}[\s\S]*?QONSUL Expertise einbeziehen/,'the initial QONSUL action is visible only before the first successful AI round');
assert.match(diagnosticUi,/\{completedAIRounds > 0 && <button[^>]+onClick=\{\(\) => addAIHypotheses\(true\)\}[\s\S]*?QONSUL Expertise vertiefen/,'deliberate regeneration uses the branded follow-up action after the first round');
...
assert.match(diagnosticUi,/QONSUL Expertise vertiefen/,'later deliberate regeneration stays available');

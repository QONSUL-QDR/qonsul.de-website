...
assert.match(diagnosticUi,/\{completedAIRounds === 0 && <button[^>]+onClick=\{\(\) => addAIHypotheses\(\)\}/,'the initial AI action is visible only before the first successful AI round');
assert.match(diagnosticUi,/\{completedAIRounds > 0 && <button[^>]+onClick=\{\(\) => addAIHypotheses\(true\)\}>Neue KI-Hypothesen erstellen/,'deliberate regeneration replaces the initial AI action after the first round');
...
assert.match(diagnosticUi,/if \(completedAIRounds >= 2 \|\| !hasAICapacity\) \{\s+setError\(''\);\s+setAnalysisExhausted\(true\);[\s\S]*?return;\s+\}\s+if \(aiSuggestions\.length > 0 && !regenerate\)[\s\S]*?if \(regenerate\) aiRequestId\.current = '';/,'a third deliberate AI attempt or a full Cause Map enters the neutral conversion state before a source event, polling, or provider request is started');
...
assert.match(diagnosticUi,/setCompletedAIRounds\(previous => Math\.min\(previous \+ 1, 2\)\);\s+setAnalysisExhausted\(false\);\s+setAiSuggestions\(accepted\)/,'a second successful AI result keeps its new hypotheses visible and does not activate conversion merely because capacity is now full');
...
assert.match(diagnosticUi,/function submitDiagnostic\(event: FormEvent<HTMLFormElement>\) \{\s+const continueAfterSave = analysisExhausted \|\| openConsultationAfterSave;\s+if \(analysisExhausted\) setOpenConsultationAfterSave\(true\);\s+void save\(event, continueAfterSave\);/,'the conversion-mode save action only sets the existing post-save consultation intent');
...
assert.match(diagnosticUi,/setAnalysisExhausted\(false\); setOpenConsultationAfterSave\(false\); setCompletedAIRounds\(0\)/,'Neu beginnen resets the conversion and completed-round states');

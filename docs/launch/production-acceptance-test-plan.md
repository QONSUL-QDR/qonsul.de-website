...
| Test | Steps | Pass criterion | Failure escalation |
|---|---|---|---|
| Sender domain verification | Confirm (via whoever holds Resend account access) that the production `CONTACT_FROM_EMAIL` domain is verified in Resend before any send attempt | Domain shows verified in Resend's dashboard | If not verified, sends will fail or land in spam — fix this first, don't test delivery against an unverified domain |

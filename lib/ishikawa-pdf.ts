...
export function ishikawaPdf(analysis: Analysis, options: { watermark?: boolean; createdAt?: Date } = {}) {
  ...
  content+='0.32 0.48 0.60 RG 2 w 74 290 m 724 290 l S 724 290 m 708 299 l 708 281 l h f\n';
  ...
  function branch(category:Category,index:number,isTop:boolean){
    ...
    causes.forEach((cause,causeIndex)=>{
      ...
      const wrapped=wrap(cause.text,31,2),baseY=isTop?py+7:py-10;
      wrapped.forEach((line,lineIndex)=>{content+=textCommand({x:left,y:baseY-lineIndex*8,size:6.5,text:line,bold:cause.source==='user'});});
      content+=textCommand({x:left,y:baseY-wrapped.length*8-1,size:5.2,text:cause.source==='user'?'EIGENE BEOBACHTUNG':cause.source==='ai'?'KI-HYPOTHESE':'REGEL-HYPOTHESE',color:[0.32,0.48,0.60]});
    });
  }

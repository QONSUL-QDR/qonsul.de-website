...
export async function syncLead(lead:Lead){
  ...
  try{
    ...
    const e=escapeHtml;
    const note=`<h2>Qonsul · Ishikawa-Analyse</h2><p><strong>${e(lead.analysis.problem)}</strong></p><p>Unternehmen: ${e(lead.company)} · Kontakt zur Analyse ausdrücklich gewünscht.</p><p>Hypothesen, keine bestätigten Ursachen. Analyse-Modus: ${lead.analysis.mode}.</p>${CATEGORIES.map(c=>`<h3>${c}</h3><ul>${lead.analysis.causes.filter(x=>x.category===c).map(x=>`<li>[${x.source==='user'?'Beobachtung':x.source==='ai'?'KI':'Regelkatalog'}] ${e(x.text)}${x.check?` — Prüfen: ${e(x.check)}`:''}${x.metric?` (${e(x.metric)})`:''}</li>`).join('')}</ul>`).join('')}<p>Interne Referenz: ${e(lead.id)}</p>`;

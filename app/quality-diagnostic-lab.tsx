...
export default function QualityDiagnosticLab({ launch }: { launch?: { problem: string; id: number } }) {
  ...
  const submissionId = useRef(''), consultationId = useRef(''), aiRequestId = useRef(''), flowId = useRef(''), completedSteps = useRef(new Set<string>()), causeInput = useRef<HTMLInputElement>(null);
  ...
  function start(value = input) {
    ...
    setInput(next); setProblem(next); setCauses([]); setAiSuggestions([]); setAvailableData([]); setSaved(null); setConsultationOpen(false); setConsultationNotice(''); setError('');
  ...
  function addCause(event: FormEvent) {
    ...
    invalidateDraftSubmission(); step('causes', 2); setCauses(previous => [...previous, { id: crypto.randomUUID(), category: selected, text, source: 'user' }]); setAiSuggestions([]); setDraft(''); setSaved(null); setError('');
  }
  function addBlindspots() {
    invalidateDraftSubmission(); const next = suggestRules(problem, causes); setCauses(previous => [...previous.filter(cause => cause.source === 'user'), ...next]); setAiSuggestions([]); setSaved(null);
    setNotice('Die ergänzten Punkte sind System-Hypothesen, keine bestätigten Ursachen. Prüfen Sie sie gezielt gegen Ihre Daten.');
  }
  async function addAIHypotheses() {
    setError(''); setNotice('');
  ...
  function acceptAISuggestion(id: string) {
    ...
    invalidateDraftSubmission(); setCauses(previous => [...previous, suggestion]); setAiSuggestions(previous => previous.filter(item => item.id !== id)); setSaved(null); setError('');
  ...
  return <section id="analyse" className="lab" aria-label="QONSUL Quality Diagnostic">
    ...
    {problem && <><div className="board-preview board-active">
      ...
      <div className="workbench"><form onSubmit={addCause} className="cause-form"><label htmlFor="diagnostic-category">Ihre Perspektive ergänzen</label><div><select id="diagnostic-category" value={selected} onChange={event => setSelected(event.target.value as Category)}>{CATEGORIES.map(category => <option key={category}>{category}</option>)}</select><input id="diagnostic-cause" ref={causeInput} value={draft} onChange={event => setDraft(event.target.value)} minLength={3} maxLength={220} required placeholder={categoryHints[selected]} aria-label="Eigene Ursache" /><button className="button button-outline">Hinzufügen +</button></div></form><div className="boost-section"><div><strong>Was übersehen wir vielleicht?</strong><p>Ergänzende Perspektiven bleiben Hypothesen und werden erst durch Daten bestätigt. Für die freiwillige KI-Unterstützung wird Ihre Eingabe vor der Anfrage bereinigt und an den KI-Dienst übermittelt. Bitte keine vertraulichen Daten eingeben.</p></div><div><button type="button" className="button button-green boost-button" disabled={busy} onClick={addAIHypotheses}>{busy ? 'KI-Hypothesen werden ergänzt …' : 'KI-Hypothesen ergänzen'}</button><button type="button" className="button button-outline boost-button" disabled={busy} onClick={addBlindspots}>Blinde Flecken ergänzen</button></div></div>{aiSuggestions.length > 0 && <section className="ai-suggestions" aria-label="KI-Hypothesen"><strong>KI-Hypothesen</strong><p>KI-Hypothesen sind Analysevorschläge und keine bestätigten Ursachen.</p><ul>{aiSuggestions.map(suggestion => <li key={suggestion.id}><div><b>KI-Hypothese · {suggestion.category}</b><span>{suggestion.text}</span>{suggestion.check && <small>{suggestion.check}</small>}</div><div><button type="button" className="button button-outline" disabled={busy} onClick={() => acceptAISuggestion(suggestion.id)}>Übernehmen</button><button type="button" className="quiet-button" disabled={busy} onClick={() => dismissAISuggestion(suggestion.id)}>Verwerfen</button></div></li>)}</ul></section>}<div aria-live="polite" className="analysis-notice">{notice}</div><div className="hypothesis-note">◎ Eigene Beobachtungen und KI-/System-Hypothesen bleiben getrennt. Eine Hypothese ist kein bestätigter Grund.</div></div>

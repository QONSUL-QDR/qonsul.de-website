...
export default function QualityDiagnosticLab({ launch }: { launch?: { problem: string; id: number } }) {
  ...
  function start(value = input) {
    ...
    flowId.current = crypto.randomUUID(); completedSteps.current.clear(); submissionId.current = ''; consultationId.current = '';
  ...
  function addCause(event: FormEvent) {
    ...
    step('causes', 2); setCauses(previous => [...previous, { id: crypto.randomUUID(), category: selected, text, source: 'user' }]); setDraft(''); setSaved(null); setError('');
  }
  function addBlindspots() {
    const next = suggestRules(problem, causes); setCauses(previous => [...previous.filter(cause => cause.source === 'user'), ...next]); setSaved(null);
  ...
  async function requestConsultation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setConsultationNotice(result.status === 'pending_review' ? 'Ihre Anfrage ist sicher vorgemerkt und wird im Cockpit geprüft.' : result.status === 'pending' ? 'Ihre Diagnostic ist gespeichert. Die Beratungsanfrage wird erneut verarbeitet.' : 'Ihre Beratungsanfrage wurde dem Quality Diagnostic zugeordnet.');
    } catch (reason) { setConsultationNotice(reason instanceof Error ? reason.message : 'Beratungsanfrage konnte nicht übermittelt werden.'); } finally { setBusy(false); }
  }
  ...
  return <section id="analyse" className="lab" aria-label="QONSUL Quality Diagnostic">
    ...
    {!problem ? <div className="lab-entry"><div className="eyebrow">DAS PROBLEM IST DER ANFANG. NICHT DAS ENDE.</div><h2>Welches Qualitätsproblem möchten Sie verstehen?</h2><p>Beschreiben Sie eine Beobachtung aus Entwicklung, Validierung oder Produktion. Bitte keine vertraulichen Daten eingeben.</p><form onSubmit={event => { event.preventDefault(); start(); }}><div className="problem-input"><span aria-hidden="true">⌕</span><input value={input} onChange={event => setInput(event.target.value)} minLength={10} maxLength={600} required placeholder="z. B. Bauteile fallen bei der thermischen Validierung sporadisch aus." /><button className="button button-green">Diagnostic starten <span>↗</span></button></div><div className="examples"><span>Zum Beispiel:</span>{examples.map(example => <button type="button" key={example} onClick={() => start(example)}>{example} <span>↗</span></button>)}</div></form></div> : <div className="active-problem"><div><span className="eyebrow">IHR QUALITÄTSPROBLEM</span><h2>{problem}</h2></div><button className="quiet-button" disabled={busy} onClick={() => { setProblem(''); setCauses([]); setAvailableData([]); setSaved(null); }}>Neu beginnen ↺</button></div>}
    {problem && <><div className="workbench"><form onSubmit={addCause} className="cause-form"><label htmlFor="diagnostic-category">Eigene Beobachtung ergänzen</label><div><select id="diagnostic-category" value={selected} onChange={event => setSelected(event.target.value as Category)}>{CATEGORIES.map(category => <option key={category}>{category}</option>)}</select><input ref={causeInput} value={draft} onChange={event => setDraft(event.target.value)} minLength={3} maxLength={220} required placeholder={categoryHints[selected]} /><button className="button button-outline">Hinzufügen +</button></div></form><div className="boost-section"><div><strong>Was übersehen wir vielleicht?</strong><p>Ergänzende Perspektiven werden als Hypothesen gekennzeichnet und müssen gegen Daten geprüft werden.</p></div><button type="button" className="button button-green boost-button" disabled={busy} onClick={addBlindspots}>Blinde Flecken ergänzen</button></div><div aria-live="polite" className="analysis-notice">{notice}</div><div className="hypothesis-note">◎ Eigene Beobachtungen und System-Hypothesen bleiben getrennt. Eine Hypothese ist kein bestätigter Grund.</div></div>
      <div className="board-preview board-active"><div className="board-heading"><span><span className="live-dot" /> STRUKTURIERTE HYPOTHESEN</span><span>{causes.filter(cause => cause.source === 'user').length} Beobachtungen · {causes.filter(cause => cause.source !== 'user').length} Hypothesen</span></div><div className="fish-categories">{CATEGORIES.map(category => <div className="cause-category" key={category}><strong>{category}</strong><ul className="cause-list">{causes.filter(cause => cause.category === category).map(cause => <li className={`cause cause-${cause.source}`} key={cause.id}><span className="cause-source">{cause.source === 'user' ? '●' : '◇'}</span>{cause.text}<button type="button" disabled={busy} onClick={() => { setCauses(previous => previous.filter(item => item.id !== cause.id)); setSaved(null); }}>Entfernen</button></li>)}</ul></div>)}</div></div>
      <section className="data-inventory"><span className="eyebrow">DIE DATENGRUNDLAGE</span><h3>Welche Daten stehen bereits zur Verfügung?</h3><p>Es werden keine Dateien hochgeladen und keine Messwerte in der Website verarbeitet.</p><div className="data-options">{DATA_KINDS.map(kind => <label key={kind}><input type="checkbox" checked={availableData.includes(kind)} onChange={event => { setAvailableData(previous => event.target.checked ? [...previous, kind] : previous.filter(item => item !== kind)); setSaved(null); }} />{kind}</label>)}</div></section>
      {causes.length > 0 && !saved && <form className="lead-form" onSubmit={save}><div className="lead-heading"><h3>Diagnostic sicher speichern</h3><p>Die Diagnostic wird separat im QONSUL Cockpit angelegt. Ohne Beratungsanfrage werden keine CRM-Kontakte oder Leads erzeugt.</p></div><div className="honey" aria-hidden="true"><label>Website<input name="website" tabIndex={-1} autoComplete="off" /></label></div><label className="check-label"><input name="diagnosticConsent" type="checkbox" required /> <span>Ich willige in die Verarbeitung meiner Diagnostic für die strukturierte Qualitätsanalyse ein. <a href="/datenschutz">Datenschutzhinweise</a></span></label>{!status.productionReady && <label className="check-label"><input name="demoConfirmed" type="checkbox" required /><span>Ich verwende ausschließlich fiktive Testdaten für diese private Vorschau.</span></label>}{error && <p role="alert" className="error-message">{error}</p>}<div className="lead-submit"><span>Keine Contact- oder CRM-Erstellung ohne die folgende ausdrückliche Anfrage.</span><button className="button button-green" disabled={busy}>{busy ? 'Diagnostic wird gesichert …' : 'Diagnostic sichern ↗'}</button></div></form>}
      {saved && <div className="lead-content"><div className="save-success" role="status"><span className="success-mark">✓</span><h3>Ihre Quality Diagnostic ist gesichert.</h3><p>Referenz: {saved.reference}. Beobachtungen, Hypothesen und Datenbasis wurden getrennt dokumentiert.</p><button className="button button-outline" type="button" onClick={() => setConsultationOpen(open => !open)}>Persönliche Beratung anfragen ↗</button></div>{consultationOpen && <form className="lead-form" onSubmit={requestConsultation}><div className="lead-heading"><h3>Optional: Persönliche Beratung</h3><p>Diese separate Einwilligung löst erst jetzt die CRM-Zuordnung aus. Kein Newsletter.</p></div><div className="lead-fields"><label>Name<input name="name" required maxLength={100} autoComplete="name" /></label><label>Geschäftliche E-Mail<input name="email" type="email" required maxLength={254} autoComplete="email" /></label><label>Unternehmen<input name="company" maxLength={150} autoComplete="organization" /></label></div><div className="honey" aria-hidden="true"><label>Website<input name="website" tabIndex={-1} autoComplete="off" /></label></div><label className="check-label"><input name="contactConsent" type="checkbox" required /> <span>Ich wünsche eine persönliche Beratung zu dieser Quality Diagnostic und stimme der Kontaktaufnahme per E-Mail zu.</span></label>{!status.productionReady && <label className="check-label"><input name="demoConfirmed" type="checkbox" required /><span>Ich verwende ausschließlich fiktive Testdaten für diese private Vorschau.</span></label>}<div className="lead-submit"><span>Die Diagnostic bleibt auch bei einer CRM-Prüfung sicher erhalten.</span><button className="button button-green" disabled={busy}>{busy ? 'Anfrage wird übermittelt …' : 'Beratung anfragen ↗'}</button></div>{consultationNotice && <p role="status" className="crm-notice">{consultationNotice}</p>}</form>}</div>}
    </>}

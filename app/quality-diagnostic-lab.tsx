...
export default function QualityDiagnosticLab({ launch }: { launch?: { problem: string; id: number } }) {
  ...
  const [hasCompletedAIRun, setHasCompletedAIRun] = useState(false);
  ...
  function start(value = input) {
    ...
    setInput(next); setProblem(next); setCauses([]); setAiSuggestions([]); setAnalysisExhausted(false); setOpenConsultationAfterSave(false); setHasCompletedAIRun(false); setBlindSpots([]); setAvailableData([]); setSaved(null); setConsultationOpen(false); setConsultationNotice(''); setError('');
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    await withSavingState(setBusy, async () => {
      try {
        ...
        step('report', 3); emitAnalyticsHook('diagnostic_completed', { diagnosticFlowId: flowId.current }); setSaved(result.diagnostic); if (openConsultationAfterSave) setConsultationOpen(true);

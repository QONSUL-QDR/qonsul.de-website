...
export default function QualityDiagnosticLab({ launch }: { launch?: { problem: string; id: number } }) {
  ...
  function start(value = input) {
    ...
    setInput(next); setProblem(next); setCauses([]); setAiSuggestions([]); setHasCompletedAIRun(false); setBlindSpots([]); setAvailableData([]); setSaved(null); setConsultationOpen(false); setConsultationNotice(''); setError('');

...
export async function POST(request:Request){
  try{
    ...
    const analyticsSessionId=optionalUuid(body.analyticsSessionId,'Analytics-Sitzung');
    ...
    const previous=await db.prepare('SELECT id, name, email, phone, company, message, privacy_version, analytics_session_id, diagnostic_flow_id, created_at, crm_status, email_status FROM contact_requests WHERE client_token_hash = ?').bind(clientTokenHash).first<{id:string;name:string;email:string;phone:string|null;company:string|null;message:string;privacy_version:string;analytics_session_id:string|null;diagnostic_flow_id:string|null;created_at:number;crm_status:string;email_status:string}>();
    if(previous){const finalCrm=previous.crm_status==='pending'?await syncContactLead({id:previous.id,name:previous.name,email:previous.email,phone:previous.phone||'',company:previous.company||'',message:previous.message,privacyVersion:previous.privacy_version,submittedAt:previous.created_at,analyticsSessionId:previous.analytics_session_id,diagnosticFlowId:previous.diagnostic_flow_id}):previous.crm_status;if(setting('PRODUCTION_READY')==='true'&&finalCrm!=='sent')return json({error:'Ihre Anfrage ist sicher vorgemerkt, konnte aber noch nicht vollständig übermittelt werden. Bitte versuchen Sie es erneut.'},503);return json({status:'accepted',reference:previous.id});}
    ...
    await db.prepare('INSERT INTO contact_requests (id, client_token_hash, name, email, phone, company, message, privacy_version, analytics_session_id, diagnostic_flow_id, created_at, expires_at, crm_status, email_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id,clientTokenHash,name,email,phone||null,company||null,message,CONTACT_PRIVACY_VERSION,analyticsSessionId,null,now,now+retention*86400000,crmStatus,emailStatus).run();
    const lead:ContactLead={id,name,email,phone,company,message,privacyVersion:CONTACT_PRIVACY_VERSION,submittedAt:now,analyticsSessionId};

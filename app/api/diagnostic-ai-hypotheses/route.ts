...
const customerError = 'Die QONSUL-Hypothesen konnten nicht ergänzt werden. Ihre eigene Analyse bleibt unverändert nutzbar.';

export async function POST(request: Request) {
  try {
    const body = await readBody(request);
    if (!await rateLimit(request, 'public-ai-hypotheses', 5)) return json({ error: 'Die QONSUL-Analyse wurde gerade bereits ausgeführt. Bitte verwenden Sie die vorhandenen Vorschläge oder versuchen Sie es in Kürze erneut.' }, 429, { 'Retry-After': '60' });
    ...
    if (error instanceof PublicAIIntakeError && error.httpStatus === 429) {
      if (error.retryAfterSeconds) return json({ error: 'Die QONSUL-Analyse wurde gerade bereits ausgeführt. Bitte verwenden Sie die vorhandenen Vorschläge oder versuchen Sie es in Kürze erneut.' }, 429, { 'Retry-After': String(error.retryAfterSeconds) });
      return json({ error: 'Die QONSUL-Analyse wurde gerade bereits ausgeführt. Bitte verwenden Sie die vorhandenen Vorschläge oder versuchen Sie es in Kürze erneut.' }, 429);
    }

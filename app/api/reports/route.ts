...
const crmDeliveryError=(status:string)=>status==='pending'
  ? 'Der Report ist sicher vorgemerkt, die Beratungsanfrage konnte vorübergehend nicht bestätigt werden. Bitte erneut versuchen.'
  : 'Der Report ist sicher vorgemerkt. Die Beratungsanfrage wird intern geprüft.';
export async function POST(request:Request){
  try{
    ...
    if(setting('PRODUCTION_READY')==='true'&&crmConsent&&finalStatus!=='sent')return json({error:crmDeliveryError(finalStatus)},503);

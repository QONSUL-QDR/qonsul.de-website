...
export default function ContactDialog({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  useEffect(()=>{if(!open){if(dialog.current?.open)dialog.current.close();return;}token.current=crypto.randomUUID();setError('');setResult(null);setBusy(false);fetch('/api/status').then(async r=>await r.json() as Status).then(setStatus).catch(()=>setStatus({productionReady:false,contactReady:false,contactEmail:'info@qonsul.de'}));dialog.current?.showModal();requestAnimationFrame(()=>firstField.current?.focus());},[open]);

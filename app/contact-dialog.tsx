...
import {CONTACT_PRIVACY_VERSION,type ContactDeliveryStatus} from '@/lib/contact';

type Status={productionReady:boolean;contactReady:boolean;contactEmail:string};
type Result={stored:boolean;crmStatus:ContactDeliveryStatus;emailStatus:ContactDeliveryStatus};
...
export default function ContactDialog({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  useEffect(()=>{if(!open){if(dialog.current?.open)dialog.current.close();return;}token.current=crypto.randomUUID();let cancelled=false;void fetch('/api/status').then(async r=>await r.json() as Status).then(value=>{if(!cancelled)setStatus(value);}).catch(()=>{if(!cancelled)setStatus({productionReady:false,contactReady:false,contactEmail:'info@qonsul.de'});});dialog.current?.showModal();requestAnimationFrame(()=>firstField.current?.focus());return()=>{cancelled=true;};},[open]);
  const close=()=>{dialog.current?.close();setError('');setResult(null);setBusy(false);setStatus(null);onClose();};
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();setBusy(true);setError('');const form=new FormData(event.currentTarget);try{const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:form.get('name'),email:form.get('email'),phone:form.get('phone'),company:form.get('company'),message:form.get('message'),website:form.get('website'),privacyAcknowledged:form.get('privacy')==='on',privacyVersion:CONTACT_PRIVACY_VERSION,demoConfirmed:form.get('demo')==='on',clientToken:token.current})});const data=await response.json() as Result&{error?:string};if(!response.ok)throw new Error(data.error||'Die Anfrage konnte nicht gesendet werden.');setResult(data);}catch(cause){setError(cause instanceof Error?cause.message:'Die Anfrage konnte nicht gesendet werden.');}finally{setBusy(false);}};
  const delivered=result?.crmStatus==='sent'&&result.emailStatus==='sent';
  return <dialog ref={dialog} className="contact-dialog" aria-labelledby="contact-title" onCancel={event=>{event.preventDefault();close();}} onClick={event=>{if(event.target===event.currentTarget)close();}}>
    <div className="contact-card">
      ...
      {result?<div className="contact-success" role="status"><span aria-hidden="true">✓</span><h3>Ihre Anfrage ist gespeichert.</h3><p>{delivered?'Die Kontaktdaten wurden im CRM gespeichert und eine Zusammenfassung wurde an info@qonsul.de gesendet.':status?.productionReady?'Ihre Anfrage liegt sicher vor. Mindestens ein externer Zustellweg benötigt eine technische Prüfung; QONSUL sieht den Status im System.':'Dies ist eine fiktive Testanfrage im privaten Vorschaudatenspeicher. Es wurde nichts an HubSpot oder per E-Mail übertragen.'}</p><button type="button" className="button button-dark" onClick={close}>Schließen</button></div>:<form onSubmit={submit}>
